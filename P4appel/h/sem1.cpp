...
void semantics(node_t *root)
{
	...
	else if(root->label == "<vars>")
	{
		...
		if(temp.token.desc != "EMPTY")
		{
			checkVar(temp);
			stackVars++;
			targetFile << "LOAD T" << findOnStack(temp) << "\n";
			targetFile << "STORE T" << findOnStack(temp) << "\n";
			semantics(root->child1);
			return;
		}
	}
	...
	else if(root->label == "<expr>")
	{
		...
		else
		{
			...
			semantics(root->child2);
			targetFile << "STORE T" << localVar << "\n";
			semantics(root->child1);
			targetFile << "ADD T" << localVar << "\n";
			return;
		}
	}
	//<A> -> <N> - <A> | <N>
	...
	else if(root->label == "<N>")
	{
		...
		else
		{
			...
			{
				targetFile << "DIV T" << localVar << "\n";
			}
			else
			{
				targetFile << "MULT T" << localVar << "\n";
			}
			return;
		}	
	}
